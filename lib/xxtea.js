...
function encrypt(data, key) {
    ...
    return Buffer.from(toUint8Array(encryptUint32Array(toUint32Array(data, false), toUint32Array(fixk(key), false)),false));
}

function encryptToString(data, key) {
    return new Buffer.from(encrypt(data, key)).toString('base64');
}

function decrypt(data, key) {
    if (typeof data === 'string') data = new Buffer.from(data, 'base64');
    if (typeof key === 'string') key = toBytes(key);
    if (data === undefined || data === null || data.length === 0) {
        return data;
    }
