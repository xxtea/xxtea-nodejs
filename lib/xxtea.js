...
function encrypt(data, key, includeLength = true) {
    if (typeof data === 'string') data = toBytes(data);
    if (typeof key === 'string') key = toBytes(key);
    if (data === undefined || data === null || data.length === 0) {
        return data;
    }
    return toUint8Array(encryptUint32Array(toUint32Array(data, includeLength), toUint32Array(fixk(key), false)), false);
}
...
function decrypt(data, key, includeLength = true) {
    if (typeof data === 'string') data = new Buffer(data, 'base64');
    if (typeof key === 'string') key = toBytes(key);
    if (data === undefined || data === null || data.length === 0) {
        return data;
    }
    return toUint8Array(decryptUint32Array(toUint32Array(data, false), toUint32Array(fixk(key), false)), includeLength);
}
